...
func mirroredWSL2Workaround(ctx context.Context, table nftables.TableRef, hairpin bool) error {
	// WSL2 does not (currently) support Windows<->Linux communication via ::1.
	if table.Family() != nftables.IPv4 {
		return nil
	}
	if !shouldInsertMirroredWSL2Rule(ctx, hairpin) {
		return nil
	}
...
//     the packet, so the loopback0 rule wouldn't be useful. However, without
