...
func (n *network) modEndpoint(ctx context.Context, epIPv4, epIPv6 netip.Addr, enable bool) error {
	if n.fw.IPv4 && epIPv4.IsValid() {
		if err := n.filterDirectAccess(ctx, n.fw.table4, n.Config4, epIPv4, enable); err != nil {
			return err
		}
	...
	if n.fw.IPv6 && epIPv6.IsValid() {
		if err := n.filterDirectAccess(ctx, n.fw.table6, n.Config6, epIPv6, enable); err != nil {
			return err
		}
...
func (n *network) filterDirectAccess(ctx context.Context, table nftables.TableRef, conf firewaller.NetworkConfigFam, epIP netip.Addr, enable bool) error {
	if n.Internal || conf.Unprotected || conf.Routed || n.fw.AllowDirectRouting {
		return nil
	}
	updater := table.ChainUpdateFunc(rawPreroutingChain, enable)
	ifNames := strings.Join(append(n.TrustedHostInterfaces, n.IfName), ", ")
