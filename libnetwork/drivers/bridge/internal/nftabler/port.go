...
func (n *network) setPerPortRules(ctx context.Context, pbs []types.PortBinding, pbc pbContext, wsl2Mirrored, enable bool) error {
	...
	if err := filterPortMappedOnLoopback(pbs, pbc, wsl2Mirrored, enable); err != nil {
		return err
	}
...
func (n *network) setPerPortForwarding(pbs []types.PortBinding, pbc pbContext, enable bool) error {
	...
	for _, pb := range pbs {
		...
		if err := updateFwdIn(fwdInPortsRuleGroup, rule); err != nil &&
...
func (n *network) setPerPortDNAT(pbs []types.PortBinding, pbc pbContext, enable bool) error {
	...
	for _, pb := range pbs {
		...
		if err := updater(initialRuleGroup, rule); err != nil {
...
func filterPortMappedOnLoopback(pbs []types.PortBinding, pbc pbContext, wsl2Mirrored, enable bool) error {
