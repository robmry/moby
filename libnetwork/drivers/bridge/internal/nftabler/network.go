...
func newNetwork(nft *nftabler, nc firewaller.NetworkConfig) (_ *network, retErr error) {
	n := &network{
		fw:            nft,
		NetworkConfig: nc,
	}
	...
	if n.fw.IPv4 {
		if err := n.configure(nft.table4, n.Config4); err != nil {
			return nil, err
		}
	}
	if n.fw.IPv6 {
		if err := n.configure(nft.table6, n.Config6); err != nil {
			return nil, err
		}
	}
	return n, nil
...
func (n *network) configure(table nftables.TableRef, conf firewaller.NetworkConfigFam) error {
	...
	ctx := log.WithLogger(context.TODO(), log.G(context.TODO()).WithFields(log.Fields{
		"bridge": n.IfName,
		"family": table.Family(),
	}))
...
func (n *network) ReapplyNetworkLevelRules() error {
	log.G(context.Background()).Warn("ReapplyNetworkLevelRules is not implemented for nftables")
	return nil
}
...
func (n *network) DelNetworkLevelRules() error {
	...
	if n.fw.IPv4 && n.Config4.Prefix.IsValid() {
		n.cleanupFam(n.fw.table4, n.Config4)
	}
	if n.fw.IPv6 && n.Config6.Prefix.IsValid() {
		n.cleanupFam(n.fw.table6, n.Config6)
	}
	return errors.Join(errs...)
}

func (n *network) cleanupFam(table nftables.TableRef, conf firewaller.NetworkConfigFam) {
	ctx := log.WithLogger(context.TODO(), log.G(context.TODO()).WithFields(log.Fields{
		"bridge": n.IfName,
		"family": table.Family(),
	}))
