...
// Package iptabler implements the bridge/internal/Firewaller interface in
// terms of iptables. If firewalld is running, it uses firewalld's (deprecated)
// direct iptables interface.
package iptabler
...
func NewIptabler(ctx context.Context, config firewaller.Config) (firewaller.Firewaller, error) {
	...
	if ipt.IPv4 {
		...
		if err := setupIPChains(ctx, iptables.IPv4, ipt.Hairpin); err != nil {
			return nil, err
		}
	...
	if ipt.IPv6 {
		...
		err := setupIPChains(ctx, iptables.IPv6, ipt.Hairpin)
		if err != nil {
			...
			iptables.OnReloaded(func() {
				...
				if err := setupIPChains(ctx, iptables.IPv6, ipt.Hairpin); err != nil {
