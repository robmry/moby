...
func testIptabler(t *testing.T, tn string, config firewaller.Config, netConfig firewaller.NetworkConfig, bindLocalhost bool, resName string) {
	...
	checkResults := func(cmd, name string, en bool) {
		...
		// - iptables-nft and iptables-legacy pick a different order
		// - if the raw table isn't used it's not included but, once it's been used, it's always
		//   included ... so, "cleaned" results differ only in the empty raw table.
		var dump string
		...
		golden.Assert(t, stripComments(dump), name+"__"+cmd+".golden")
	}
	...
	// Add the network.
	nw, err := fw.NewNetwork(netConfig)
	assert.NilError(t, err)
