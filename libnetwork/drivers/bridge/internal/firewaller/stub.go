...
type StubFirewaller struct {
	Config
	// A real firewaller shouldn't track its own networks, the bridge driver is doing that, and
	// a firewalld reload is assumed to drop config for all networks (so, the bridge driver will
	// just drop all of its firewaller.Networks). But, for unit tests cross-checking the driver,
	// this is useful ...
	Networks map[string]*StubFirewallerNetwork
	FFD      bool // filter forward drop
}
...
func (fw *StubFirewaller) NewNetwork(nc NetworkConfig) (Network, error) {
	if nw, ok := fw.Networks[nc.IfName]; ok && !nw.Deleted {
		return nil, fmt.Errorf("StubFirewaller: network with IfName %q already exists", nc.IfName)
	}
	nw := &StubFirewallerNetwork{NetworkConfig: nc}
	fw.Networks[nc.IfName] = nw
	return nw, nil
}

func (fw *StubFirewaller) FilterForwardDrop(ipv IPVersion) error {
	fw.FFD = true
	return nil
}
...
func (nw *StubFirewallerNetwork) ReapplyNetworkLevelRules() error {
	return nil
}

func (nw *StubFirewallerNetwork) DelNetworkLevelRules() error {
	nw.Deleted = true
	return nil
}
