...
func (n *Namespace) AddInterface(ctx context.Context, srcName, dstPrefix string, options ...IfaceOption) error {
	...
	err = func() error {
		...
		return setInterfaceMAC(ctx, nlh, iface, i)
	}()
	if err != nil {
	...
	opts := netlink.LinkSubscribeOptions{
		...
		ErrorCallback: func(err error) {
			log.G(ctx).WithFields(log.Fields{
				"ifi":   ifIndex,
				"error": err,
			}).Debug("netlink error while waiting for interface up")
		},
	}
	if ns.IsOpen() {
		opts.Namespace = &ns
	}
	if err := netlink.LinkSubscribeWithOptions(update, upped, opts); err != nil {
