...
// advertiseAddrs triggers the kernel to send gratuitous ARP and
// Neighbour Advertisement messages, so that caches are updated with
// the current MAC address associated with the interface's IP addresses.
//
// IP addresses are recycled quickly when endpoints are dropped on
// network disconnect or container stop. A new MAC address may have
// been generated, so this is necessary to avoid packets sent to the
// old MAC address getting dropped until the ARP/Neighbour cache
// entries expire.
//
