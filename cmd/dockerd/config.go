...
func installCommonConfigFlags(conf *config.Config, flags *pflag.FlagSet) {
	...
	flags.StringVar(&conf.HostGatewayIP, "host-gateway-ip", "", "IP address that the special 'host-gateway' string in --add-host resolves to. Defaults to the IP address of the default bridge")
