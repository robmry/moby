...
func (n *network) configure(ctx context.Context, table nftables.Table, conf firewaller.NetworkConfigFam) (nftables.Modifier, error) {
	...
	if err := tm.Apply(ctx); err != nil {
		return nftables.Modifier{}, fmt.Errorf("adding rules for bridge %s: %w", n.config.IfName, err)
	}
...
func (n *network) DelNetworkLevelRules(ctx context.Context) error {
	remove := func(remover nftables.Modifier) {
		if remover.IsValid() {
			ctx := log.WithLogger(ctx, log.G(ctx).WithFields(log.Fields{"bridge": n.config.IfName}))
	...
	remove(n.remover4)
	n.remover4 = nftables.Modifier{}
	remove(n.remover6)
	n.remover6 = nftables.Modifier{}
	return nil
}
