...
func TestDaemonConfigurationHostGatewayIP(t *testing.T) {
	tests := []struct {
		...
		},
		{
			...
			flags:     []string{"hello"},
			expSetErr: `invalid argument "hello" for "--host-gateway-ip" flag: IP address is not correctly formatted: hello`,
		},
		{
			name:   "config bad address",
			config: `{"host-gateway-ips": ["hello"]}`,
			expErr: `merged configuration validation from file and command line flags failed: host gateway IP address is not correctly formatted: hello`,
		},
		{
			name:   "config not array",
			config: `{"host-gateway-ips": "192.0.2.1"}`,
			expErr: `json: cannot unmarshal string into Go struct field Config.host-gateway-ips of type []string`,
		},
		{
	...
	for _, tc := range tests {
		t.Run(tc.name, func(t *testing.T) {
			...
			configFile := makeConfigFile(t, tc.config)
			flags := pflag.NewFlagSet("test", pflag.ContinueOnError)
			flags.Var(opts.NewNamedListOptsRef("host-gateway-ips", &c.HostGatewayIPs, opts.ValidateIPAddress),
			...
			if tc.expErr != "" {
				...
				assert.NilError(t, err)
				assert.Check(t, is.DeepEqual(cc.HostGatewayIPs, tc.expVal))
